 ...
 * the cap-string matrix, we enforce role gates: org_owner/org_admin and
 * ops/sales can write inventory; accounting/viewer/customer_user are
 * read-only.
 */
export function requireCap(caller: Caller, cap: string): void {
  ...
  if (role === 'ops' || role === 'sales') return;
  if (!isWrite && (role === 'accounting' || role === 'viewer')) return;
