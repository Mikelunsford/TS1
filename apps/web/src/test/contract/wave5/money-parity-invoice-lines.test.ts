...
/**
 * Invoice-line money parity (Wave 5 / Phase 7). Mirrors
 * `wave4/money-parity-quote-lines.test.ts` for the invoice surface — the BE
 * handler (`supabase/functions/invoicing-api/handlers/line-items.ts#computeLineTotals`)
 * uses the IDENTICAL math as the quote-line handler:
 *
 *   gross            = roundHalfEven(qty * unit_price_cents)
 *   line_total_cents = gross - discount_cents
 *   tax_amount_cents = roundHalfEven(line_total_cents * tax_rate)
 *
 * The SPA `taxTotalCents` mirrors this per-line. This file pins the 3-line
 * fixture against both sides so a drift between invoice and quote rounding
 * (or a regression to half-up) fails loudly.
 */

describe('money parity: invoice-line per-line totals (3-line fixture)', () => {
  it('matches the BE computeLineTotals output byte-for-byte', () => {
    // Line 1: 4 * 2500 = 10000; no discount; rate 0.0875 → 10000 * 0.0875 = 875.
    ...
    // SPA-side preview via the shared helper.
  ...
  it('single-line discount is applied before tax (invoice surface)', () => {
  ...
  it('rounding is per-line, not per-invoice', () => {
    // Two identical lines: 1 * 333 = 333; rate 0.085 → 333 * 0.085 = 28.305 -> 28 per line.
    // Per-line: 28 + 28 = 56. Document-rounded would be round(666 * 0.085) = 57.
    // The canonical answer is 56 — proves the BE recompute trigger rolls up
    // per-line totals, not a document-wide reroll.
    const lines: TaxableLine[] = [
